...
impl TextEditor {
    ...
    pub fn select_line(&mut self, row: usize) {
        ...
        if row + 1 < self.lines.len() {
            ...
            self.cursor = Pos::new(row, char_len(&self.lines[row]));
        }
    }
    ...
    pub fn cut(&mut self) {
        let s = self.selected_text();
        if !s.is_empty() { self.clipboard = s; self.delete_selection(); self.ensure_visible_default(); }
    }
    ...
    pub fn move_right(&mut self, extend: bool) {
        ...
        else if self.cursor.row + 1 < self.lines.len() { self.cursor.row += 1; self.cursor.col = 0; }
        if !extend { self.clear_selection(); }
        self.ensure_visible_default();
    }
    ...
    pub fn move_down(&mut self, extend: bool) {
        ...
        if self.cursor.row + 1 < self.lines.len() {
            self.cursor.row += 1;
            self.cursor.col = self.cursor.col.min(char_len(&self.lines[self.cursor.row]));
        } else { self.cursor.col = char_len(&self.lines[self.cursor.row]); }
        if !extend { self.clear_selection(); }
        self.ensure_visible_default();
    }
    ...
    pub fn move_home(&mut self, extend: bool) {
        self.begin_selection(extend);
        let first_nws = self.lines[self.cursor.row].chars().take_while(|c| *c == ' ').count();
        self.cursor.col = if self.cursor.col == first_nws { 0 } else { first_nws };
        if !extend { self.clear_selection(); }
        self.ensure_visible_default();
    }
    ...
    pub fn page(&mut self, down: bool, extend: bool) {
        ...
        self.begin_selection(extend);
        if down { self.cursor.row = (self.cursor.row + page).min(self.lines.len() - 1); }
        else { self.cursor.row = self.cursor.row.saturating_sub(page); }
        self.cursor.col = self.cursor.col.min(char_len(&self.lines[self.cursor.row]));
        if !extend { self.clear_selection(); }
        self.ensure_visible_default();
    }
    ...
    pub fn scroll_by(&mut self, rows: i32) {
        ...
        self.scroll_row = (self.scroll_row as i32 - rows).clamp(0, max as i32) as usize;
    }

    fn ensure_visible_default(&mut self) {
        if let Some(g) = self.last_geom {
            let vcols = self.visible_cols(g);
            self.ensure_visible(g.visible_rows, vcols);
        }
    }

    fn visible_cols(&self, g: EditorGeom) -> usize {
    ...
    fn ensure_visible(&mut self, visible_rows: usize, visible_cols: usize) {
        ...
        if self.cursor.row < self.scroll_row { self.scroll_row = self.cursor.row; }
        else if self.cursor.row >= self.scroll_row + visible_rows {
            self.scroll_row = self.cursor.row + 1 - visible_rows;
        }
        if visible_cols > 0 {
            if self.cursor.col < self.scroll_col { self.scroll_col = self.cursor.col; }
    ...
    pub fn pos_at(&self, x: f32, y: f32) -> Option<Pos> {
        ...
        let col_raw = (rel_x / g.char_w).round() as usize;
        let col = (self.scroll_col + col_raw).min(char_len(&self.lines[row]));
    ...
    pub fn click(&mut self, x: f32, y: f32, extend: bool) {
        ...
        if extend {
            self.begin_selection(true);
            self.cursor = p;
            self.ensure_visible_default();
            return;
        }

        const MAX_CLICK_GAP:  std::time::Duration = std::time::Duration::from_millis(400);
        const MAX_CLICK_DIST: f32 = 6.0;

        let now = std::time::Instant::now();
        let close_enough = self.last_click_pos
            .map(|(lx, ly)| (lx - x).abs() <= MAX_CLICK_DIST && (ly - y).abs() <= MAX_CLICK_DIST)
            .unwrap_or(false);
        let in_time = self.last_click_at
            .map(|t| now.duration_since(t) <= MAX_CLICK_GAP)
            .unwrap_or(false);

        self.click_count = if close_enough && in_time { (self.click_count + 1).min(3) } else { 1 };
    ...
    pub fn build_items(
        ...
        let vis_rows  = (((rh - theme.px(12.0)) / line_h).floor() as usize).max(1);

        let geom = EditorGeom { rect, text_x, top_y, char_w, line_h, visible_rows: vis_rows };
        ...
        push_rect(items, (rx + gutter_w - theme.px(1.0), ry), (theme.px(1.0), rh), theme::BORDER);
        ...
        for (vi, row) in (self.scroll_row..last_row).enumerate() {
            ...
            if sel.is_none() && row == self.cursor.row {
                push_rect(items, (rx + gutter_w, ly), (rw - gutter_w, line_h), theme::CURRENT_LINE);
            }

            if let Some((s, e)) = sel {
                if row >= s.row && row <= e.row {
                    let sc = if row == s.row { s.col } else { 0 };
                    let ec = if row == e.row { e.col } else { char_len(&self.lines[row]) + 1 };
                    let sx = text_x + sc.saturating_sub(self.scroll_col) as f32 * char_w;
                    let ex = text_x + ec.saturating_sub(self.scroll_col) as f32 * char_w;
                    let w  = (ex - sx).max(char_w * 0.35);
                    push_rect(items, (sx, ly), (w, line_h), theme::SELECTION_BG);
                }
            }
...
fn plain_line(line: &str, scroll_col: usize, font: &Arc<Font>, font_px: f32) -> Vec<Span> {
