...
use super::draw::{
    push_border, push_border_clipped, push_ellipse_outline, push_ellipse_outline_rotated,
    push_rrect, push_rrect_clipped,
};
...
impl Ui {
    pub fn slider(
        ...
        push_rrect(
            &mut self.items,
            [r[0] + thumb_d_half, track_y, usable_w, track_h],
            track_h * 0.5,
            theme::CONTROL_BG,
        );
        if filled_w > 0.0 {
            push_rrect(
                &mut self.items,
                [r[0] + thumb_d_half, track_y, filled_w, track_h],
                track_h * 0.5,
                theme::ACCENT,
            );
        }
        ...
        let thumb_color = if hovered || self.drag_owner == Some(id) {
            theme::ACCENT_HI
        } else {
            theme::ACCENT
        };
    ...
    pub fn drag_float(
        ...
        self.drag_float_clipped(id, r, value, speed, label, None)
    }

    pub fn drag_float_clipped(
        &mut self,
        id: WidgetId,
        r: Rect,
        value: f32,
