...
impl TextEditor {
    ...
    pub(crate) fn selected_text(&self) -> String {
        ...
        if s.row == e.row {
            return substr(&self.lines[s.row], s.col, e.col);
        }
    ...
    pub(crate) fn delete_selection(&mut self) -> bool {
        ...
        if start.row == end.row {
            ...
            self.lines.splice(
                start.row..=end.row,
                std::iter::once(format!("{head}{tail}")),
            );
        }
        self.cursor = start;
        self.anchor = None;
    ...
    pub fn scroll_by_rows(&mut self, delta: i32) {
        if delta < 0 {
            ...
            self.scroll_row =
                (self.scroll_row + delta as usize).min(self.lines.len().saturating_sub(1));
        }
    }
    ...
    pub fn scroll_by_cols(&mut self, delta: i32) {
        if delta < 0 {
            ...
            let max_col = self.lines.iter().map(|l| char_len(l)).max().unwrap_or(0);
